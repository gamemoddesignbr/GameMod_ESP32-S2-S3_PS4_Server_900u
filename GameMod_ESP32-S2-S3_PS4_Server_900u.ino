...
String AP_SSID = "GameMod_PS4_WiFi";
...
boolean useWF = false;
IPAddress WIFI_LOCALIP(192, 168, 0, 250);
IPAddress WIFI_GATEWAY(192, 168, 0, 1);
IPAddress WIFI_SUBNET(255, 255, 255, 0);
IPAddress WIFI_PDNS(0, 0, 0, 0);
IPAddress WIFI_SDNS(0, 0, 0, 0);
...
void handleConfig(AsyncWebServerRequest * request) {
  if (request -> hasParam("ap_ssid", true) && request -> hasParam("ap_pass", true) && request -> hasParam("web_ip", true) && request -> hasParam("web_port", true) && request -> hasParam("subnet", true) && request -> hasParam("wifi_ssid", true) && request -> hasParam("wifi_pass", true) && request -> hasParam("wifi_host", true) && request -> hasParam("usbwait", true)) {
    ...
    String WIFI_LOCALIP = request -> getParam("wifi_local_ip", true) -> value();
    String WIFI_GATEWAY = request -> getParam("wifi_gateway", true) -> value();
    String WIFI_SUBNET = request -> getParam("wifi_subnet", true) -> value();
    ...
    if (request -> hasParam("usewifistaticip", true)) {
      tmpuw = "true";
    }
    ...
    if (request -> hasParam("useled", true)) {
      tmpUseLED = "true";
    }
...
void setup() {
  ...
  if (FILESYS.begin(SS, SPI)) {
    ...
    if (FILESYS.begin(true)) {
      ...
      if (FILESYS.exists("/config.ini")) {
        ...
        if (iniFile) {
          ...
          if (instr(iniData, "WIFI_LOCALIP=")) {
            String strsLocalIp = split(iniData, "WIFI_LOCALIP=", "\r\n");
            strsLocalIp.trim();
            WIFI_LOCALIP.fromString(strsLocalIp);
          }

          if (instr(iniData, "WIFI_GATEWAY=")) {
            String strsGateway = split(iniData, "WIFI_GATEWAY=", "\r\n");
            strsGateway.trim();
            WIFI_GATEWAY.fromString(strsGateway);
          }

          if (instr(iniData, "WIFI_SUBNET=")) {
            String strsSubnet = split(iniData, "WIFI_SUBNET=", "\r\n");
            strsSubnet.trim();
            WIFI_SUBNET.fromString(strsSubnet);
          }

          if (instr(iniData, "WIFI_PDNS=")) {
            String strsPDNS = split(iniData, "WIFI_PDNS=", "\r\n");
            strsPDNS.trim();
            WIFI_PDNS.fromString(strsPDNS);
          }

          if (instr(iniData, "WIFI_SDNS=")) {
            String strsSDNS = split(iniData, "WIFI_SDNS=", "\r\n");
            strsSDNS.trim();
            WIFI_SDNS.fromString(strsSDNS);
          }

          if (instr(iniData, "USEWF=")) {
            String struw = split(iniData, "USEWF=", "\r\n");
            struw.trim();
            if (struw.equals("true")) {
              useWF = true;
            } else {
              useWF = false;
            }
    ...
    //The board is functioning as an access point
    if (startAP) {
      ...
      WiFiStarted = WiFi.softAP(AP_SSID.c_str(), AP_PASS.c_str());
    ...
    //The board is functioning connected to an existing WiFi router
    if (connectWifi && WIFI_SSID.length() > 0 && WIFI_PASS.length() > 0) {
      //WiFi.setAutoConnect(true);
      WiFi.setAutoReconnect(true);

      if (useWF) {
        if (!WiFi.config(WIFI_LOCALIP, WIFI_GATEWAY, WIFI_SUBNET, WIFI_PDNS, WIFI_SDNS)) {
          //HWSerial.println("Failed to configure WiFi");
        }
      }
  ...
  if (WiFiStarted) {
    if ((useLED) && (LED_PIN > 0)) {
      ...
      if (startAP) {
        ...
        if (currentMillis - heartbeatMillis >= 750) {
          //restart this TIMER
          heartbeatMillis = currentMillis;
          digitalWrite(LED_PIN, !digitalRead(LED_PIN));
        }
      } else {
        //Connected to a WiFi router... LED solid.
        digitalWrite(LED_PIN, HIGH);
      }
    }
    else
